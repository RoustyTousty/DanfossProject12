...
public class CsvResultRepository : IResultRepository
{
    ...
    public async Task SaveAsync(IResultData resultData, string filepath)
    {
        await SaveManyAsync(new List<IResultData> { resultData }, filepath);
    }


    public async Task SaveManyAsync(List<IResultData> resultDataList, string filepath)
    {
        try
        {
        ...
        catch (IOException ex)
        {
            ...
            throw new Exception($"Failed to write multiple results to CSV file: {filepath}", ex);
        }

        Console.WriteLine($"Saved data successfully into {filepath}");
    }

    public async Task<List<IResultData>> GetAllAsync(string filepath)
    {
        ...
        {
            throw new Exception($"Failed to read results from CSV file: {filepath}", ex);
        }
    }


    public async Task<List<IResultData>> GetByTimeRangeAsync(DateTime timeFrom, DateTime timeTo, string filepath)
    {
        List<IResultData> Data = await GetAllAsync(filepath);
